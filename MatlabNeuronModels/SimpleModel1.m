...
% pulse of input DC current, with repeat option
repeat = 100;
stim_delay  = 2; % ms
stim_period = .025; % ms (25 us)
stim_left   = tend - repeat * (stim_delay + stim_period); 
I_stim = params(type_no, 10) * 40;
I      = [zeros(1, stim_delay*fs), I_stim * ones(1, stim_period*fs)];
I      = repmat(I,1,repeat);
I      = [I, zeros(1, stim_left*fs)]; % pA
...
for i = 1:N-1 
    ...
    % applying the solution to have sharp spike upstroke
%     if (v <= vt)
%         k = 0.7;
%     elseif(v > vt)
%         k = 7;
